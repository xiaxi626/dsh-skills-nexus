...
/**
 * Tests for the `list` SOURCE column and the pure `sourceLabel` derivation.
 *
...
/** Capture everything `list` writes to stdout (mirrors test/toggle.test.ts). */
async function captureList(argv: string[]): Promise<{ code: number; out: string }> {
  const writes: string[] = []
