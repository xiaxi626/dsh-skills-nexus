...
const TEMPLATES: Record<Shell, string> = {
  bash: bashTemplate,
  zsh: zshTemplate,
  fish: fishTemplate,
 ...
 * that asked for it.
 */
export function parseCompletionsArgs(argv: string[]): CompletionsOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (flag === '--shell') {
      ...
      throw new Error(
        `unknown argument "${a}"; usage: dsh-skills-nexus completions --shell <${SUPPORTED_SHELLS.join('|')}>`,
      )
    }
  }
  ...
  if (shell === undefined) {
    throw new Error(
      `--shell is required; usage: dsh-skills-nexus completions --shell <${SUPPORTED_SHELLS.join('|')}>`,
    )
  }
