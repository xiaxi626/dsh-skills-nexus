...
export async function list(_argv) {
    ...
    process.stdout.write(`    ${'NAME'.padEnd(nameW)}  ${'SOURCE'.padEnd(srcW)}  ${'SUBDIR'.padEnd(subW)}  REF           COMMIT    DIR      UPDATED\n`);
    for (const r of rows) {
        process.stdout.write(`${r[0]}  ${r[1].padEnd(nameW)}  ${r[2].padEnd(srcW)}  ${r[3].padEnd(subW)}  ${r[4].padEnd(13)}${r[5].padEnd(9)}${r[6].padEnd(9)}${r[7]}\n`);
    }
