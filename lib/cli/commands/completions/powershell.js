...
export const powershellTemplate = [
    "# PowerShell completion for dsh-skills-nexus - enable with:",
    ...
    "  # PS 5.1). $done counts finished tokens, so the slot being typed is $done-2.",
    "  $els = @($commandAst.CommandElements | ForEach-Object { $_.ToString() })",
    "  $done = $els.Count",
    "  if ($done -gt 0 -and $els[$done - 1] -eq $wordToComplete) { $done = $done - 1 }",
    "  $slot = $done - 2",
    "",
    "  $candidates = @()",
    "  if ($slot -lt 0) {",
