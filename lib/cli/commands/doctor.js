...
import { MANIFEST_PATH, NEXUS_HOME, REPOS_DIR, OFFICIAL_SKILLS_DIR, repoDir, } from '../../paths.js';
 ...
 */
export async function runChecks(includeUpdates) {
    const { entries, trusted, check: manifestCheck } = await checkManifest();
    const checks = [manifestCheck, await checkRoots(), await checkSymlinks(entries)];
    ...
    if (includeUpdates)
        checks.push(await checkUpdates(entries));
...
function isWellFormedEntry(e) {
    if (typeof e !== 'object' || e === null)
        return false;
    const o = e;
    return (isNonEmptyString(o.name) &&
...
async function checkRoots() {
    ...
    if (issues.length === 0) {
        ...
        if (readable.length > 0)
            parts.push(`${readable.join(' and ')} readable`);
        if (absent.length > 0)
            parts.push(`${absent.join(' and ')} not created yet`);
...
async function checkSymlinks(entries) {
    const issues = [];
    let ok = 0;
    let disabled = 0;
...
async function checkOrphanRepos(entries) {
    const orphans = await findOrphanRepos(entries);
    const issues = orphans.map((p) => ({
        severity: 'warn',
        code: 'orphan-repo',
        name: `repos/${p}`,
...
async function checkGitSanity(entries) {
    const issues = [];
    let withGit = 0;
    for (const e of entries) {
        try {
    ...
    const detail = entries.length === 0 ? 'no clones' : `${withGit}/${entries.length} clones have .git`;
 ...
 */
async function checkUpdates(entries) {
    const issues = [];
    for (const e of entries) {
...
/* Aggregation                                                         */
/* ------------------------------------------------------------------ */
function finalize(id, detail, issues) {
    let status = 'ok';
    if (issues.some((i) => i.severity === 'error'))
        status = 'error';
    else if (issues.some((i) => i.severity === 'warn'))
        status = 'warn';
    const check = { id, status, issues };
    if (detail !== undefined)
        check.detail = detail;
    return check;
}
...
export function formatHuman(report) {
    ...
    for (const c of report.checks) {
        ...
        if (c.issues.length === 0) {
            ...
            for (const i of rest)
                lines.push(`${' '.repeat(idW + 8)}${humanIssueText(i)}`);
            for (const i of c.issues)
                if (i.fix)
                    fixes.push(i.fix);
        }
    ...
    if (s.updates > 0)
        parts.push(`${s.updates} update(s) available`);
    lines.push(`${parts.join(', ')}.`);
    if (fixes.length > 0) {
        lines.push('Fix hints:');
        for (const f of dedupe(fixes))
            lines.push(`  - ${f}`);
    }
    return `${lines.join('\n')}\n`;
}
