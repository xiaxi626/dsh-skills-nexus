...
npm test            # node:test + tsx — expected: 111 tests, all passing
...
echo "--- [d] resolveAll: only alpha-skill and beta-skill ---"
...
| `[d]` resolveAll | only `alpha-skill` + `beta-skill` | root docs (README.zh-CN.md etc.) are not skills |
...
| `[g]` large guard | without `--yes`: prompt then abort; with `--yes`: 21 skills registered | guard works in both directions |
| `[h]` flat-md rules | `with-name` + `sub-skill` listed, `plain.md` absent | no frontmatter flat md ≠ skill; SKILL.md always counts |

Note: `resolveAll` lists *all* entries registered in the current `$DSH_HOME`,
...
1. **Same `$DSH_HOME` accumulates entries** — steps `[b]`–`[h]` share one
   demo `DSH_HOME`; later `resolveAll`/`list` outputs include earlier entries.
...
- **DSH runtime integration** — provider `list()`/`get()` are unchanged;
  `subdir` is a new optional manifest field (old manifests load fine).
